...
if __name__ == "__main__":
    ...
    if args.mode=="precomputation":
        ...
        f = open(f'{args.output_path}/{dataset_class}.qtoken2var.json', 'w')
        f.write(json.dumps(qtoken2var))
        f.close()

        f = open(f'{args.output_path}/{dataset_class}.qtoken2std.json', 'w')
        f.write(json.dumps(qtoken2std))
        f.close()

        f = open(f'{args.output_path}/{dataset_class}.qtoken2did.json', 'w')
        ...
        qtoken2var = json.load(open(f'{args.output_path}/{dataset_class}.qtoken2var.json'))
        qtoken2std = json.load(open(f'{args.output_path}/{dataset_class}.qtoken2std.json'))
        qtoken2did = json.load(open(f'{args.output_path}/{dataset_class}.qtoken2did.json'))
        ...
        for qpp_name in args.qpp_names:
            qpp_bank = {}
            with open(f"{args.output_path}/{dataset_name}.{query_type}-{qpp_name}", 'r') as r:
                for line in r:
            ...
            ppl_bank = {}
            with open(f"{args.output_path}/{dataset_name}.{query_type}-ppl-{args.LM}", 'r') as r:
                for line in r:
                    qid, pp_value = line.rstrip().split()
                    if qid in qrel:
    ...
    if args.mode!="precomputation":
        ...
        for name in name_list:
            output_path_ = f"{args.output_path}/{dataset_name}.{query_type}-{name}"
